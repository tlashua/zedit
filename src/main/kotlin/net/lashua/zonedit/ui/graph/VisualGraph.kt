...
private val log = LoggerFactory.getLogger("net.lashua.zonedit.ui.graph.VisualGraph")
...
data class VisualGraph<N, E>(
    ...
    fun findNodeAt(x: Float, y: Float): VisualNode<N>? {
        log.debug("VisualGraph: Finding node at position ({}, {})", x, y)
        log.debug("VisualGraph: There are {} nodes in the graph", nodes.size)

        // Log all nodes in the graph
        nodes.forEachIndexed { index, node ->
            log.debug("VisualGraph: Node {} at index {}: id={}, position=({}, {}), size=({}, {})",
                index, node.id, node.x, node.y, node.width, node.height)
        }

        // Check nodes in reverse order to prioritize nodes drawn on top
        val node = nodes.reversed().find { node ->
            val contains = node.containsPoint(x, y)
            log.debug("VisualGraph: Node {} at ({}, {}) with size ({}, {}) contains point: {}",
                node.id, node.x, node.y, node.width, node.height, contains)
            contains
        }

        log.debug("VisualGraph: Found node at position ({}, {}): {}", x, y, node?.id)
        return node
    }
