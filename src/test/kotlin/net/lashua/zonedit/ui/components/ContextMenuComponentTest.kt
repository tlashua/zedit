...
class ContextMenuComponentTest {
    ...
    // Note: Testing onDismiss is challenging with the current API
    // In a real-world scenario, we might need to modify the component to make it more testable
    ...
    fun connectionContextMenu_shouldNotShowWhenConnectionIsNull() {
        ...
        // Verify the menu is not shown - we can't directly assert it doesn't exist
        // Instead, we'll use a try-catch to verify it throws an exception when we try to find it
        var menuFound = false
        try {
            composeRule.onNodeWithText("Delete Connection").assertIsDisplayed()
            menuFound = true
    ...
    @Test
    fun connectionContextMenu_shouldNotShowWhenPositionIsNull() {
        // Create test data
        val sourceRoom = Room(
            id = "test0",
            name = "Source Room",
        ...
        // Set up the composition with null position
        composeRule.setContent {
            ContextMenuComponent.ConnectionContextMenu(
                connectionTriple = connectionTriple,
                position = null,
